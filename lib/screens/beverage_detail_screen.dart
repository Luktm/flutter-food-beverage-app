...
class BeverageDetailScreen extends StatefulWidget {
  static const routeName = "/beverage_detail_screen";
  final String id;

  const BeverageDetailScreen({Key? key, required this.id}) : super(key: key);

  @override
  State<BeverageDetailScreen> createState() => _MealDetailScreenState();
}

class _MealDetailScreenState extends State<BeverageDetailScreen>
  ...
  void initState() {
    super.initState();
    context.read<FNBDetailCubit>().fetchDrinkDetailById(id: widget.id);
      ...
      body: BlocBuilder<FNBDetailCubit, FNBDetailState>(
        builder: (context, state) {
          if (state is FNBDetailFetched) {
            final Drinks drinkDetail = state.drinkDetail.drinks![0];
            List<IngredientMeasure> populateIngredient =
                _populateIngredientMeasure(drinkDetail);
  ...
  List<IngredientMeasure> _populateIngredientMeasure(Drinks mealDetail) {
    return [
      IngredientMeasure(
